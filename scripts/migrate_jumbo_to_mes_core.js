#!/usr/bin/env node
/**
 * migrate_jumbo_to_mes_core.js
 * Drop jumbo tables from public (empty), recreate in mes_core schema.
 */
'use strict';
...
const pool = new Pool({
  ...
  options:  '-c search_path=mes_core',
});

const DROP_SQL = `
  DROP TABLE IF EXISTS public.jumbo_box_items;
  DROP TABLE IF EXISTS public.jumbo_packing_boxes;
  DROP TABLE IF EXISTS public.jumbo_assembly_components;
  DROP TABLE IF EXISTS public.jumbo_assemblies;
  DROP TABLE IF EXISTS public.jumbo_serials;
  DROP TABLE IF EXISTS public.jumbo_serial_batches;
`;
...
async function run() {
  try {
    ...
    console.log('dropped public.jumbo_* tables');
    await pool.query(CREATE_SQL);
    console.log('created mes_core.jumbo_* tables');
  } catch (err) {
