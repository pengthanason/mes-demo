...
export interface WorkflowResult {
  ...
  result: 'PASS' | 'FAIL';
  total_sec: number;
  line: 'internal' | 'external';   // สายที่บันทึกผล (แท็บ Internal/External) — เก่าที่ไม่มี → internal
  created_at: string;
}
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<WorkflowResult[]> => {
      ...
      return ((res.data as any)?.data ?? []).map((r: any) => ({
        ...
        total_sec: Number(r.total_sec) || 0, line: r.line === 'external' ? 'external' : 'internal', created_at: r.created_at,
      }));
    },
  });
}
...
export function useWorkflowResultCreate() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (input: { serial: string; customer: string; model: string; sequence: string; result: string; total_sec: number; line?: 'internal' | 'external'; steps?: { process: string; result: string }[] }) => {
